...
const Navbar = () => {
 ...
 }, [userId, fetchCount, notificationCount]); 
  ...
  const handleClick = (item) => {
    ...
    if (item === "Jobs/Internship") {
      navigate("/jobs");
    }
    if (item === "Roadmap") {
      navigate("/roadmap");
    }
    if (item === "Skill Gap Analysis") {
      navigate("/skill-gap");
    }
  ...
  return (
    <nav className="flex font-Poppins justify-between items-center w-full h-16 px-6 bg-[#698C85] bg-opacity-40 backdrop-blur-md shadow-md sticky top-0 z-50">
      <h1 className="text-xl font-semibold tracking-wider w-auto">
        CAREER CLIMB
      </h1>
      ...
      <ul
        className={`${isOpen ? "flex" : "hidden"} absolute top-16 ${
          isOpen ? "right-0 w-1/2" : ""
        } bg-background bg-opacity-95 md:bg-transparent md:static md:flex md:flex-row flex-col md:items-center md:space-x-5 shadow-md md:shadow-none transition-all duration-300`}
      >
        {isUser &&
          NavItemUser.map((item, index) => (
            <li
              key={index}
              className="px-2 py-3 md:py-2 cursor-pointer hover:border-b-2 hover:border-y-zinc-950 hover:font-medium"
        ...
        {isEmployer &&
          NavItemEmployer.map((item, index) => (
            <li
              key={index}
              className="px-2 py-3 md:py-2 cursor-pointer hover:border-b-2 hover:border-y-zinc-950 hover:font-medium"
              onClick={() => handleClick(item)}
        ...
        {(isUser || isEmployer) && (
          <div className="flex-col space-x-5 md:hidden">
            ...
            <button className="flex items-start justify-center justify-items-center hover:scale-105 my-2">
              ...
              <li
                className="px-2 md:py-2 cursor-pointer hover:border-b-2 hover:border-y-zinc-950 hover:font-medium"
      ...
      {profileClicked && (
        <div className="absolute top-16 right-0 bg-green-opacity-50 bg-opacity-95 shadow-xl rounded-md w-48 h-28 p-2 flex-col items-start justify-between">
          <h1 className="text-center border-b-2 border-gray-700 font-semibold">
            Yusuf Reza Hasnat
          </h1>
          <ul className="flex flex-col items-start justify-start mt-2">
            ...
            >
              ...
              <span className="text-lg font-normal px-2 hover:underline">
                View Profile
              </span>
            </button>
            <button className="ml-4 hover:scale-105" onClick={handleLogout}>
